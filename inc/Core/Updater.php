...
class Updater {
    ...
    public function __construct() {
        if ( defined('FD_MODULE_INTER_DEV_MODE') ) {
            add_filter( 'https_ssl_verify', '__return_false' );
            add_filter( 'https_local_ssl_verify', '__return_false' );
            add_filter( 'http_request_host_is_external', '__return_true' );
        }

        $this->plugin_slug = FD_MODULE_INTER_SLUG;
        $this->version = FD_MODULE_INTER_VERSION;
        ...
        if (isset($_GET['flexify_checkout_module_inter_check_updates']) && $_GET['flexify_checkout_module_inter_check_updates'] === '1') {
            ...
            if ($remote_data) {
                ...
                if (version_compare($current_version, $latest_version, '<')) {
                    $message = esc_html__('Uma nova versão do plugin Flexify Checkout - Inter addon está disponível.', 'module-inter-bank-for-flexify-checkout');
                    ...
                    $message = esc_html__('A versão do plugin Flexify Checkout - Inter addon é a mais recente.', 'module-inter-bank-for-flexify-checkout');
                    $class = 'notice-success';
                ...
                $message = esc_html__('Não foi possível verificar atualizações para o plugin Flexify Checkout - Inter addon.', 'module-inter-bank-for-flexify-checkout');
                $class = 'notice-error';
