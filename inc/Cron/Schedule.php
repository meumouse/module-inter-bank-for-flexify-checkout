...
class Schedule {
    ...
    public function __construct() {
		...
		add_action( 'module_inter_bank_check_interpixautomatico', array( $this, 'check_interpix_automatico' ) );
    	add_action( 'module_inter_bank_check_interpixautomatico_order', array( $this, 'check_interpix_automatico_order' ) );
    ...
    public function maybe_create_crons() {
		...
		if ( ! wp_next_scheduled('module_inter_bank_check_interpixautomatico') ) {
			wp_schedule_event( time(), 'hourly', 'module_inter_bank_check_interpixautomatico' );

			do_action('module_inter_bank_check_interpixautomatico');
		}
    }
	...
	/**
	 * Check Inter pix automatic payments.
	 *
	 * Queries for pending orders using the Pix Automático payment method and queues
	 * each order for individual checking.
	 *
	 * @since 1.4.0
	 * @return void
	 */
	public function check_interpix_automatico() {
		$order_ids = $this->get_pending_orders( 'interpixautomatico' );
		$this->set_logger_source( 'interpixautomatico', false );
		$this->log( 'Consulting automatic Pix orders via Cron: ' . print_r( $order_ids, true ) );

		foreach ( $order_ids as $order_id ) {
			WC()->queue()->add(
				'module_inter_bank_check_interpixautomatico_order',
				array(
					'order_id' => $order_id,
				),
				'module-inter-bank-for-flexify-checkout'
	...
	/**
	 * Check if an automatic Pix payment order has been completed.
	 *
	 * Processes an individual order, checks its payment status with the Inter bank
	 * API and updates the order status accordingly.
	 *
	 * @since 1.4.0
	 * @param int $order_id | Order ID
	 * @return void
	 */
	public function check_interpix_automatico_order( $order_id ) {
		try {
			$this->set_logger_source( 'interpixautomatico', false );
			$this->log( 'Consulting automatic Pix order #' . $order_id );
			$order = wc_get_order( $order_id );

			if ( ! $order ) {
				throw new \Exception( 'Order #' . $order_id . ' not found in automatic Pix query.' );
			}
			...
			if ( in_array( $status, array( 'AUTORIZADA', 'CONCLUIDA', 'LIQUIDADA' ), true ) ) {
				$order->payment_complete();
				$order->add_order_note( __( 'Automatic Pix payment confirmed (delayed) by Inter bank.', 'module-inter-bank-for-flexify-checkout' ) );
			} elseif ( in_array( $status, array( 'CANCELADA', 'NAO_AUTORIZADA', 'REJEITADA', 'EXPIRADA' ), true ) ) {
				$order->update_status( 'cancelled', __( '[EXPERIMENTAL] Automatic Pix charge not authorized or expired.', 'module-inter-bank-for-flexify-checkout' ) );
			}
		} catch ( \Exception $e ) {
			$this->log( 'Error consulting automatic Pix order #' . $order_id . '. ' . $e->getMessage() );
		}
	}
