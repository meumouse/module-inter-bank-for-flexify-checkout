...
class Error_Handler {
    ...
    public function __construct( $plugin_slug ) {
        ...
        $this->log_file = FD_MODULE_INTER_PATH . 'error_log.txt';
    ...
    public function handle_error( $errno, $errstr, $errfile, $errline ) {
        ...
        if ( strpos( $errfile, FD_MODULE_INTER_PATH ) !== false ) {
    ...
    public function handle_exception( $exception ) {
        ...
        if ( strpos( $exception->getFile(), FD_MODULE_INTER_PATH ) !== false ) {
    ...
    public function handle_shutdown() {
        ...
        if ( $last_error && ( $last_error['type'] & ( E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR ) ) ) {
            ...
            if ( strpos( $last_error['file'], FD_MODULE_INTER_PATH ) !== false ) {
    ...
    private function deactivate_plugin( $error = '' ) {
        ...
        add_action( 'admin_notices', function () use ( $error ) {
            ...
            $message = sprintf( __( '<strong>Flexify Checkout - Inter addon</strong> foi desativado devido a um erro crítico: %s - Verifique o arquivo de logs para mais detalhes.', 'module-inter-bank-for-flexify-checkout' ), esc_html( $error ) );
