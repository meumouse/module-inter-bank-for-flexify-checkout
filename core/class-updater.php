...
namespace MeuMouse\Flexify_Checkout\Inter_Bank\Updater;
...
class Updater {
    ...
    public function __construct() {
        if ( defined('FCW_MODULE_INTER_DEV_MODE') ) {
            add_filter( 'https_ssl_verify', '__return_false' );
            add_filter( 'https_local_ssl_verify', '__return_false' );
            add_filter( 'http_request_host_is_external', '__return_true' );
        }

        $this->plugin_slug = FCW_MODULE_INTER_SLUG;
        $this->version = FCW_MODULE_INTER_VERSION;
    ...
    public function add_check_updates_link( $actions, $plugin_file ) {
        if ( $plugin_file === $this->plugin_slug . '/' . $this->plugin_slug . '.php' ) {
            $check_updates_link = '<a href="' . esc_url( add_query_arg( 'flexify_checkout_module_inter_check_updates', '1' ) ) . '">' . esc_html__( 'Verificar atualizações', 'module-inter-bank-for-flexify-checkout' ) . '</a>';
        ...
        if (isset($_GET['flexify_checkout_module_inter_check_updates']) && $_GET['flexify_checkout_module_inter_check_updates'] === '1') {
            ...
            if ($remote_data) {
                ...
                if (version_compare($current_version, $latest_version, '<')) {
                    $message = esc_html__('Uma nova versão do plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce está disponível.', 'module-inter-bank-for-flexify-checkout');
                    ...
                    $message = esc_html__('A versão do plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce é a mais recente.', 'module-inter-bank-for-flexify-checkout');
                    $class = 'notice-success';
                ...
                $message = esc_html__('Não foi possível verificar atualizações para o plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce.', 'module-inter-bank-for-flexify-checkout');
                $class = 'notice-error';
