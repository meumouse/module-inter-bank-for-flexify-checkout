 ...
 */
if ( !class_exists('Module_Inter_Bank_Updater') ) {
    class Module_Inter_Bank_Updater {
        ...
        public function __construct() {
            if ( defined( 'FLEXIFY_CHECKOUT_MODULE_INTER_DEV_MODE' ) ) {
                add_filter( 'https_ssl_verify', '__return_false' );
                add_filter( 'https_local_ssl_verify', '__return_false' );
                add_filter( 'http_request_host_is_external', '__return_true' );
            }

            $this->plugin_slug = FLEXIFY_CHECKOUT_MODULE_INTER_SLUG;
            $this->version = FLEXIFY_CHECKOUT_MODULE_INTER_VERSION;
            $this->cache_key = 'flexify_checkout_module_inter_check_updates';
            $this->cache_data_base_key = 'flexify_checkout_module_inter_remote_data';
        ...
        public function add_check_updates_link( $actions, $plugin_file ) {
            if ( $plugin_file === $this->plugin_slug . '/' . $this->plugin_slug . '.php' ) {
                $check_updates_link = '<a href="' . esc_url( add_query_arg( 'flexify_checkout_module_inter_check_updates', '1' ) ) . '">' . esc_html__( 'Verificar atualizações', 'flexify-checkout-for-woocommerce' ) . '</a>';
                $actions['flexify_checkout_module_inter_check_updates'] = $check_updates_link;
            }
            ...
            if (isset($_GET['flexify_checkout_module_inter_check_updates']) && $_GET['flexify_checkout_module_inter_check_updates'] === '1') {
                ...
                if ($remote_data) {
                    ...
                    if (version_compare($current_version, $latest_version, '<')) {
                        $message = esc_html__('Uma nova versão do plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce está disponível.', 'flexify-checkout-for-woocommerce');
                        ...
                        <script type="text/javascript">
                            if (!sessionStorage.getItem('reload_flexify_checkout_module_inter_update')) {
                                sessionStorage.setItem('reload_flexify_checkout_module_inter_update', 'true');
                                window.location.reload();
                            }
                        </script>
                        <?php
                    } elseif (version_compare($current_version, $latest_version, '>=')) {
                        $message = esc_html__('A versão do plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce é a mais recente.', 'flexify-checkout-for-woocommerce');
                        $class = 'notice-success';
                    ...
                    $message = esc_html__('Não foi possível verificar atualizações para o plugin Módulo adicional banco Inter para Flexify Checkout para WooCommerce.', 'flexify-checkout-for-woocommerce');
                    $class = 'notice-error';
